  ...
  useEffect(() => {
    if (account && ref.current) {
      ...
      ref.current.appendChild(
        jazzicon(width, parseInt(account.address.toJSON().slice(2, 10), 16))
      );
    }
  }, [account]);
  return (
              ...
              <>
                <Menu.Button className="items-center flex-row w-auto rounded-3xl bg-[#983BF6] gap-1 p-1 text-sm font-semibold text-white flex">
                  <div className="flex items-center gap-2">
                    <Identicon width={30} />
                    <div>{`${wallet.account?.address
                      ?.toString()
                      .slice(0, 6)}...${wallet.account?.address
                      ?.toString()
                      ?.slice(-5)}`}</div>
                    {open ? (
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-1">
                          ...
                          <IconButtons
                            onClick={() => {
                              window.open(
                                `https://devnet.venomscan.com/accounts/${wallet.account?.address}`,
                                "_blank"
                              );
                            }}
                            icon={out}
