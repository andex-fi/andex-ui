...
export class ManageTokenStore {
  ...
  public changeData<K extends keyof ManageTokenStoreData>(
    key: K,
    value: ManageTokenStoreData[K]
  ): void {
    this.data[key] = value;
  }

  public changeState<K extends keyof ManageTokenStoreState>(
    key: K,
    value: ManageTokenStoreState[K]
  ): void {
  ...
  protected async handleWalletAddressChange(
    walletAddress?: string,
    prevWalletAddress?: string
  ): Promise<void> {
  ...
  public async mint(): Promise<void> {
    ...
    try {
      await new rpc.Contract(
        TokenAbi.Root,
        new Address(this.state.tokenRoot)
      ).methods
        .mint({
      ...
      const intervalId = setInterval(async () => {
        ...
        if (this.data.targetWalletBalance !== currentTargetWalletBalance) {
          this.changeData("token", {
            ...this.data.token!,
            totalSupply: this.data.token?.totalSupply
              ? new BigNumber(this.data.token.totalSupply)
                  .plus(
                    new BigNumber(this.data.amountToMint).shiftedBy(
  ...
  public async burn(): Promise<void> {
    ...
    const result = await rpc.packIntoCell({
      structure: [{ name: "data", type: "bytes" } as const] as const,
      data: {
      ...
      await new rpc.Contract(
        ...
        .burnTokens({
          amount: new BigNumber(this.data.amountToBurn)
            .shiftedBy(Number(this.data.token.decimals))
            .toFixed(),
        ...
        if (this.data.targetWalletBalance !== currentTargetWalletBalance) {
          this.changeData("token", {
            ...
            totalSupply: this.data.token?.totalSupply
              ? new BigNumber(this.data.token.totalSupply)
                  .minus(
                    new BigNumber(this.data.amountToBurn ?? 0).shiftedBy(
                      +this.data.token!.decimals
                    )
                  )
                  .decimalPlaces(
  ...
  public async loadTargetWalletBalance(): Promise<void> {
    ...
    try {
      ...
      this.changeData(
        "targetWalletBalance",
        new BigNumber(balance).shiftedBy(-this.data.token.decimals).toFixed()
      );
    } catch (e) {
