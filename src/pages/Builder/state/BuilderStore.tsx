...
export class BuilderStore {
   ...
   */
  public changeData<K extends keyof BuilderStoreData>(
    key: K,
    value: BuilderStoreData[K]
  ): void {
    this.data[key] = value;
  }

  protected changeState<K extends keyof BuilderStoreState>(
    key: K,
    value: BuilderStoreState[K]
  ): void {
  ...
  protected async loadTokensData(): Promise<Token[]> {
    ...
    try {
      const result = await Promise.all(
        tokenRoots.map((tokenRoot) =>
          TokenWallet.getTokenFullDetails(tokenRoot)
        )
      );
      tokens = (
        result.filter((token, idx) => {
