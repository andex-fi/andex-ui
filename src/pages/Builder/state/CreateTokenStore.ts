...
export class CreateTokenStore {
   ...
   * @protected
   */
  protected transactionResult: CreateTokenTransactionResult | undefined =
    undefined;
   ...
   */
  public changeData<K extends keyof CreateTokenStoreData>(
    key: K,
    value: CreateTokenStoreData[K]
  ): void {
    this.data[key] = value;
  }
   ...
   */
  protected changeState<K extends keyof CreateTokenStoreState>(
    key: K,
    value: CreateTokenStoreState[K]
  ): void {
   ...
   * @protected
   */
  protected handleWalletAccountChange(
   ...
   * @protected
   */
  protected handleCreateTokenSuccess({
    input,
    transaction,
  }: CreateTokenSuccessResult): void {
  ...
  public async createToken(): Promise<void> {
    ...
    try {
      ...
      await new rpc.Contract(TokenAbi.Factory, TokenFactoryAddress).methods
        .createToken({
          callId,
          name: this.name.trim(),
          symbol: this.symbol.trim(),
          decimals: this.decimals.trim(),
          initialSupplyTo: new AddressLiteral(
