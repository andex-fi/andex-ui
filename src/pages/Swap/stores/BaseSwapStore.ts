...
> extends BaseStore<T, U> {
  ...
  public get isLeftAmountValid(): boolean {
    ...
    return (
      ...
      this.leftBalanceNumber.gte(this.leftAmountNumber)
    );
  }
  ...
  public get isRightAmountValid(): boolean {
    ...
    return (
      this.rightAmount.length > 0 && isGoodBignumber(this.rightAmountNumber)
    );
  }
  ...
  protected get leftTokenAddress(): Address | undefined {
    return this.leftToken?.root !== undefined
  ...
  protected get rightTokenAddress(): Address | undefined {
    return this.rightToken?.root !== undefined
  ...
  protected async prepare(): Promise<void> {
    ...
    if (this.pair?.address !== undefined) {
      ...
      try {
        ...
        await Promise.all([this.syncPairBalances(), this.syncPairData()]);
  ...
  protected async calculateLeftToRight(force: boolean = false): Promise<void> {
    ...
    if (
      ...
      if (this.pair?.contract !== undefined) {
        try {
          const { expected_amount: expectedAmount, expected_fee: fee } =
  ...
  protected finalizeCalculation(): void {
    ...
    const pairLeftBalanceNumber = this.isPairInverted
      ? this.pairRightBalanceNumber
      : this.pairLeftBalanceNumber;
    const pairRightBalanceNumber = this.isPairInverted
      ? this.pairLeftBalanceNumber
    ...
    const expectedLeftPairBalanceNumber =
      pairLeftBalanceNumber.plus(amountNumber);
    const expectedRightPairBalanceNumber =
      pairRightBalanceNumber.minus(expectedAmountNumber);
  ...
  protected async syncPairData(): Promise<void> {
    ...
    const [
      { left, right },
      {
        value0: { denominator, pool_numerator: numerator },
      },
    ] = await Promise.all([
      this.pair.contract.methods
        .getTokenRoots({
          answerId: 0,
        })
        .call({
  ...
  protected async syncPairBalances(): Promise<void> {
    ...
    try {
      ...
      this.setData("pair", {
        ...this.pair,
        balances: {
          left: balances.left_balance.toString(),
          right: balances.right_balance.toString(),
        },
      });
    } catch (e) {
