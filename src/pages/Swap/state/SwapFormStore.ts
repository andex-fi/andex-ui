...
import { NPoolsList } from '../../../constants/config'
import { useRpc, useStaticRpc } from '../../../hooks'
import type { LiquidityStablePoolData } from '../../../constants'
import {
...
import { DEFAULT_SLIPPAGE_VALUE, RECEIPTS } from '../constants'
import { useSwapApi } from '../hooks/useApi'
import type { BaseSwapStoreData } from './BaseSwapStore'
...
import { calcCrossExchangeSlippage } from '../utils'
        ...
        try {
            ...
            if (this.data.transfer?.method.toLowerCase() === 'transfer') {
                message = await TokenWalletUtils.transfer(this.leftToken?.wallet as unknown as Address, {
                    ...
                    recipient: resolveVenomAddress(data?.input.recipient as string),
                    remainingGasTo: this.wallet.account!.address,
                }, {
                ...
                message = await wrappedCoinVaultContract(this.options.wrappedCoinVaultAddress)
                    .methods.wrap({
                        ...
                        owner_address: resolveVenomAddress(data?.input.owner_address as string),
                        payload: data?.input.payload as string,
                    ...
                    this.setData({
                        ...
                        route: {
                            ...
                            steps: response.steps.map(step => ({
                                ...
                                poolAddress: resolveVenomAddress(step.poolAddress),
                ...
                if (this.direction === SwapDirection.LTR) {
                    if (isGoodBignumber(rightAmount ?? 0)) {
                        this.setData({
                            ...
                            route: {
                                leftAmount: this.data.leftAmount,
                                recipientWalletAddress: resolveVenomAddress(response.walletOfDestination),
                                rightAmount: rightAmount.toFixed(),
                                slippage: calcCrossExchangeSlippage(this.data.slippage, steps.length),
