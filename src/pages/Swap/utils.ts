...
export async function getExpectedSpendAmount(
  ...
  DecodedAbiFunctionOutputs<typeof DexAbi.Pair, "expectedSpendAmount">
> {
  return pairContract.methods
    .expectedSpendAmount({
      answerId: 0,
...
export function getExchangePerPrice(
  ...
  return value.div(dividedBy).shiftedBy(decimals).dp(0, BigNumber.ROUND_DOWN);
}

export function getDirectExchangePriceImpact(
  start: BigNumber,
  end: BigNumber
): BigNumber {
...
export function getCrossExchangeSlippage(
  value: string,
  stepsCounts: number
): string {
...
export function getReducedCrossExchangeFee(
  iteratee: SwapRouteStep[]
): BigNumber {
  ...
  const reduced = steps.reduce<{
    ...
    (acc, step, idx) => {
      ...
      const pairLeftBalanceNumber = isInverted
        ? new BigNumber(step.pair.balances?.right || 0).shiftedBy(
            -(step.pair.decimals?.right ?? 0)
          )
        : new BigNumber(step.pair.balances?.left || 0).shiftedBy(
            -(step.pair.decimals?.left ?? 0)
          );
      ...
      const end = expectedRightPairBalanceNumber.div(
        expectedLeftPairBalanceNumber
      );

      return {
        leftRoot: isInverted
          ? step.pair.roots?.left.toString()
          : step.pair.roots?.right.toString(),
        value: end
          .minus(start)
          .div(end)
          .abs()
