...
type PoolFormShape = {
  ...
  onChangeRightAmount: (
    value: AddLiquidityFormStoreData["rightAmount"]
  ) => void;
...
export function useAddLiquidityForm(): PoolFormShape {
  ...
  const onChangeLeftAmount: PoolFormShape["onChangeLeftAmount"] = async (
    value
  ) => {
    await formStore.changeLeftAmount(value);
  };

  const onChangeRightAmount: PoolFormShape["onChangeRightAmount"] = async (
    value
  ) => {
    await formStore.changeRightAmount(value);
  };

  const onSelectLeftToken: PoolFormShape["onSelectLeftToken"] = async (
    root
  ) => {
    hideTokensList();
  ...
  const onSelectRightToken: PoolFormShape["onSelectRightToken"] = async (
    root
  ) => {
    hideTokensList();
