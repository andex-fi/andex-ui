...
export class TokensCacheService extends BaseStore<
  TokensCacheData,
  TokensCacheState
> {
  constructor(
    ...
    reaction(
      ...
      async ([time, tokens], [prevTime, prevTokens]) => {
    ...
    this._tokensBalancesSubscribers = new Map<
      string,
      Subscription<"contractStateChanged">
    >();
    this._tokensBalancesSubscribersMutex = new Mutex();
  ...
  /**
   * Returns only verified tokens and tokens were vendor is Andex
   */
  public get verifiedAndexTokens(): TokenCache[] {
    return this.tokens.filter(
      (token) => token.verified && token.vendor === "andex"
    );
  }
  ...
  public remove(token: TokenCache): void {
    this.setData(
      "tokens",
      this.tokens.filter((item) => item.root !== token.root)
    );
  }
   ...
   */
  public update<K extends keyof TokenCache & string>(
    root: string,
    key: K,
    value: TokenCache[K]
  ): void {
  ...
  public isCustomToken(root: string): boolean {
    return (
      this.tokensList.tokens.find((token) => token.address === root) ===
  ...
  public isTokenUpdatingBalance(root?: string): boolean {
    return (
      (root === undefined
        ? false
        : this.state.updatingTokensBalance.get(root)) || false
    );
  }
  ...
  public isTokenUpdatingWallet(root?: string): boolean {
    return (
      (root === undefined
        ? false
        : this.state.updatingTokensWallet.get(root)) || false
    );
  }
  ...
  public async search(query: string): Promise<TokenCache[]> {
    const filtered = this.tokens.filter(
      (token) =>
  ...
  public async syncToken(root: string, force?: boolean): Promise<void> {
    ...
    if (
      token === undefined ||
      (!force &&
        (this.isTokenUpdating(root) ||
          Date.now() - (token.updatedAt || 0) < 60 * 1000))
    ) {
      return;
    }
    ...
    this.setState(
      "updatingTokens",
      new ObservableMap(this.state.updatingTokens).set(root, true)
    );
    ...
    this.setState(
      "updatingTokens",
      new ObservableMap(this.state.updatingTokens).set(root, false)
    );
  }
  ...
  public async watch(root?: string, prefix: string = ""): Promise<void> {
    ...
    if (token.wallet !== undefined) {
      ...
      await this._tokensBalancesSubscribersMutex.use(async () => {
        const entry = this._tokensBalancesSubscribers.get(key);
        ...
        const subscription = (
          await rpc.subscribe("contractStateChanged", {
            address,
          })
        ).on("data", async (event) => {
          debug(
        ...
        this._tokensBalancesSubscribers.set(key, subscription);

        debug(
          `%cRPC%c Subscribe to the %c${
            token.symbol
          }%c token wallet %c${sliceAddress(
            token.wallet
          )}%c balance updates with key %c${prefix}-${sliceAddress(root)}`,
          "font-weight: bold; background: #4a5772; color: #fff; border-radius: 2px; padding: 3px 6.5px",
          "color: #c5e4f3",
          "color: #bae701",
          "color: #c5e4f3",
  ...
  public async unwatch(root?: string, prefix: string = ""): Promise<void> {
    ...
    await this._tokensBalancesSubscribersMutex.use(async () => {
      const subscriber = this._tokensBalancesSubscribers.get(key);
      ...
      this._tokensBalancesSubscribers.delete(key);

      debug(
        `%cRPC%c Unsubscribe from the %c${
          token.symbol
        }%c token wallet %c${sliceAddress(
          token.wallet
        )}%c balance updates with key %c${prefix}-${sliceAddress(root)}`,
  ...
  public async syncTokenBalance(root: string): Promise<void> {
    if (
      ...
      this.isTokenUpdatingBalance(root)
    ) {
      return;
    }
    ...
    try {
      this.setState(
        "updatingTokensBalance",
        new ObservableMap(this.state.updatingTokensBalance).set(root, true)
      );
      ...
      warn(
        "Cannot update token balance. Wallet account of this token not created yet ->",
        e.message
      );
      this.update(root, "balance", undefined);
    } finally {
      this.setState(
        "updatingTokensBalance",
        new ObservableMap(this.state.updatingTokensBalance).set(root, false)
      );
    }
  }
  ...
  public async syncTokenWalletAddress(root: string): Promise<void> {
    if (
      root === undefined ||
      this.wallet.account?.address === undefined ||
      this.isTokenUpdatingWallet(root)
    ) {
      return;
    }
    ...
    if (token.wallet === undefined) {
      this.setState(
        "updatingTokensWallet",
        new ObservableMap(this.state.updatingTokensWallet).set(root, true)
      );
      ...
      try {
        ...
        this.setState(
          "updatingTokensWallet",
          new ObservableMap(this.state.updatingTokensWallet).set(root, false)
        );
      }
    }
  }
   ...
   * @private
   */
  _tokensBalancesSubscribers: Map<string, Subscription<"contractStateChanged">>;
   ...
   * @private
   */
  _tokensBalancesSubscribersMutex: Mutex;
}
