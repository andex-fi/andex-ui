...
import { error, WalletService } from "@andex/sdk";
...
export class FavoritePairs {
  ...
  public remove(address: string, silent?: boolean): void {
    ...
    const index = this.state.data[this.wallet.address].findIndex(
  ...
  public readFromStorage(): Data {
    ...
    try {
      ...
      return Object.entries(data as { [key: string]: unknown }).reduce(
        (acc, [key, value]) => {
  ...
  public filterData(query?: string): PairData[] {
    ...
    const data = query
      ? addresses.filter(
          ({ name }) => name && name.toLocaleLowerCase().indexOf(query) > -1
        )
      : addresses;
