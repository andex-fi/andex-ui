...
import { getRpc, useRpc } from "../hooks/useRpc";
...
export type WalletNativeCoin = Pick<
  Token,
  "balance" | "decimals" | "icon" | "name" | "symbol"
>;
...
export class WalletService extends BaseStore<WalletData, WalletState> {
  ...
  public get isOutdated(): boolean {
    if (
      ...
      this.options?.minWalletVersion === undefined
    ) {
      return false;
    }
   ...
   */
  public get walletContractCallbacks():
    | Contract<typeof DexAbi.Callbacks>
    | undefined {
  ...
  protected async init(): Promise<void> {
    ...
    console.log("initialising:", staticRpc);
    ...
    const permissionsSubscriber = await staticRpc.subscribe(
      "permissionsChanged"
    );
  ...
  protected async onAccountChange(
    ...
    try {
      const state = await getFullContractState(account.address, staticRpc);
