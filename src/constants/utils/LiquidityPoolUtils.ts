...
export type LiquidityPoolConnectCallbacks = SendMessageCallback &
  TransactionCallbacks<
...
export type LiquidityPoolCreateCallbacks = SendMessageCallback &
  TransactionCallbacks<
...
// deposit
export type LiquidityPoolDepositSuccessResult =
  | DexPairDepositLiquiditySuccess
  | DexPairDepositLiquiditySuccessV2;

export type LiquidityPoolDepositCallbacks = SendMessageCallback &
  TransactionCallbacks<
...
export type LiquidityPoolDepositParams = {
  ...
  Omit<DexAccountDepositLiquidityParams, "callId" | "sendGasTo">;

// withdraw
export type LiquidityPoolWithdrawCallbacks = SendMessageCallback &
  TransactionCallbacks<
...
export abstract class LiquidityPoolUtils {
  ...
  public static async connect(
    ...
    try {
      ...
      const message = await DexAccountUtils.addPair(
        params.dexAccountAddress,
        {
  ...
  public static async create(
    ...
    try {
      const stream = await subscriber
        .transactions(resolveVenomAddress(params.dexRootAddress))
        .flatMap((item) => item.transactions)
        .filter(
          (tx) =>
        ...
        .filterMap(async (tx) => {
          const events = await dexRootContract(
          ...
          if (
            ...
            address !== undefined &&
            addressesComparer(
              resolveVenomAddress(params.expectedAddress),
              address
            )
          ) {
      ...
      const message = await DexUtils.deployPair(
        params.dexRootAddress,
        {
          leftRootAddress: params.leftRootAddress,
          rightRootAddress: params.rightRootAddress,
  ...
  public static async depositLiquidity(
    ...
    try {
      ...
      const message = await DexAccountUtils.depositLiquidity(
        params.dexAccountAddress,
        {
    ...
    try {
      const stream = await subscriber
        ...
        .filterMap(async (tx) => {
          ...
          if (
            decodedTx?.method === "dexPairWithdrawSuccess" &&
      ...
      const message = await PairUtils.withdrawLiquidity(
        params.dexRootAddress,
        {
  ...
  public static async pools(
    ...
    return Promise.allSettled(
      addresses.map((poolAddress) =>
        LiquidityPoolUtils.get(poolAddress, walletOwnerAddress)
      )
    ).then((results) =>
      results.map((result) =>
