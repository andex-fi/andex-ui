...
export type DexPairDepositLiquiditySuccessV2 = DecodedAbiFunctionInputs<
  typeof DexAbi.DexPairCallbacks,
...
export type DexPairExchangeSuccess = DecodedAbiFunctionInputs<
  typeof DexAbi.DexPairCallbacks,
  "dexPairExchangeSuccess"
>;

export type DexPairExchangeSuccessV2 = DecodedAbiFunctionInputs<
  typeof DexAbi.DexPairCallbacks,
  "dexPairExchangeSuccessV2"
>;

export type DexPairOperationCancelled = DecodedAbiFunctionInputs<
  typeof DexAbi.DexPairCallbacks,
  "dexPairOperationCancelled"
>;

export type DexPairWithdrawSuccess = DecodedAbiFunctionInputs<
  typeof DexAbi.DexPairCallbacks,
  "dexPairWithdrawSuccess"
>;

export type DexPairWithdrawSuccessV2 = DecodedAbiFunctionInputs<
  typeof DexAbi.DexPairCallbacks,
...
export abstract class PairUtils {
  ...
  public static async withdrawLiquidity(
    ...
    const [leftRootUserWalletAddress, rightRootUserWalletAddress] =
      await Promise.all([
        params.leftRootUserWalletAddress ??
          (await TokenWalletUtils.walletAddress(
            {
        ...
        params.rightRootUserWalletAddress ??
          (await TokenWalletUtils.walletAddress(
            {
    ...
    const allDeployed =
      leftRootUserWalletState?.isDeployed &&
      rightRootUserWalletState?.isDeployed;
    const deployWalletGrams =
      params.deployWalletGrams == null ? "100000000" : params.deployWalletGrams;

    const payload = await PairUtils.buildWithdrawLiquidityPayload(
      pairAddress,
      {
    ...
    return TokenWalletUtils.transferToWallet(
      lpUserWalletAddress,
      {
        ...
        remainingGasTo: resolveVenomAddress(params.userAddress),
      },
      {
        amount: "2700000000",
        bounce: true,
  ...
  > {
    return (
      await dexPairContract(pairAddress)
  ...
  public static async check(
    ...
    if (!pairState?.isDeployed) {
      debug(
        `%cRPC%c Check pair %c${sliceAddress(
  ...
  public static async getDetails(
    ...
    const result: PairFullDetails = {
      address: resolveVenomAddress(pairAddress),
      balances,
      feeParams,
      roots,
      state,
    ...
    if (
      leftTokenDetails?.decimals !== undefined &&
      rightTokenDetails?.decimals !== undefined
    ) {
      result.decimals = {
    ...
    if (
      leftTokenDetails?.symbol !== undefined &&
      rightTokenDetails?.symbol !== undefined
    ) {
      result.symbols = {
  ...
  public static async isActive(
    ...
    DecodedAbiFunctionOutputs<typeof DexAbi.Pair, "isActive">["value0"]
  > {
    return (
      await dexPairContract(pairAddress)
